...
The simulation was executed in parallel via WSL2. To bypass known OpenMPI shared memory and `MPI_THREAD_MULTIPLE` compatibility issues within the WSL environment, the environment variables were explicitly passed to a single-node MPI launch. Furthermore, the output was forced to Legacy VTK format to prevent XML parsing corruption.
